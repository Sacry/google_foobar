...
The time for the mass escape has come, and you need to distract the
guards so that the bunny prisoners can make it out! Unfortunately for
you, they're watching the bunnies closely. Fortunately, this means
they haven't realized yet that the space station is about to explode
due to the destruction of the LAMBCHOP doomsday device. Also
fortunately, all that time you spent working as first a minion and
then a henchman means that you know the guards are fond of
...
2 (the one with 3 bananas wins and gets 3 bananas from the
loser). After the second round, they will have 4 and 4 (the one with 6
bananas loses 2 bananas). At that point they stop and get back to
guarding.
...
Write a function answer(banana_list) which, given a list of positive
...
class Graph(object):
    ...
    @staticmethod
    def build(banana_list):
        g = Graph()

        for i in xrange(len(banana_list)):
            for j in xrange(i+1, len(banana_list)):
                if is_inf(banana_list[i], banana_list[j]):
                    g.add_edge(i, j)

        return g

    def add_vertex(self, no):
        if no not in self.vertices:
            self.vertices[no] = Vertex(no)

        return self.vertices[no]

    def add_edge(self, i, j):
        vertexi = self.add_vertex(i)
        vertexj = self.add_vertex(j)

        vertexi.add_neighbor(vertexj)
        vertexj.add_neighbor(vertexi)

    def max_connected_pairs(self):
        i = 0
        while self.vertices:
            # vertex with least degree
            v1 = min(self.vertices.values(), key=lambda x: x.degree)
            v2 = next(iter(v1.neighbors))
            self.remove_pair(v1.no, v2.no)
            i += 2
        return i

    def remove_vertex(self, no):
    ...
    def remove_pair(self, i, j):
        ...
        # remove vertices with no neighbor
        for k in self.vertices.keys():
            if len(self.vertices[k].neighbors) == 0:
                del self.vertices[k]


def answer(banana_list):
    g = Graph.build(banana_list)
    return len(banana_list) - g.max_connected_pairs()
